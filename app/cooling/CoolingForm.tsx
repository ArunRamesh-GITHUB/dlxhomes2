...
function Form() {
  ...
  const campaignRef = searchParams.get("ref") ?? "";
    ...
    try {
      if (formspreeId) {
        await fetch(`https://formspree.io/f/${formspreeId}`, {
          ...
          body: JSON.stringify({ ...form, campaign_reference: campaignRef, source: "cooling_letter" }),
        });
      }
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-7 sm:p-10">
      {campaignRef && (
        <div className="mb-6 px-4 py-2.5 rounded-lg bg-amber-50 border border-amber-200 text-xs text-dlx-gold">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
        ...
        <div>
          ...
          <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="07700 900000" className={inputClass} />
        </div>
        <div>
          <label className="block text-xs font-medium text-stone-500 mb-2 tracking-wide">
            Property Postcode <span className="text-dlx-gold">*</span>
          </label>
          <input type="text" name="postcode" value={form.postcode} onChange={handleChange} required placeholder="SW1A 1AA" className={`${inputClass} uppercase`} />
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <textarea name="message" value={form.message} onChange={handleChange} rows={3} placeholder="E.g. which rooms get hottest, whether you have AC already, any planning or leasehold questions..." className={`${inputClass} resize-none`} />
      </div>

      <button type="submit" disabled={submitting} className="w-full flex items-center justify-center gap-2.5 py-4 rounded-xl bg-dlx-gold text-white font-semibold text-base hover:bg-dlx-gold-light transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed">
        {submitting ? (
          <>
...
export default function CoolingForm() {
  return (
    <Suspense fallback={<div className="p-10 text-stone-400 text-sm text-center">Loading...</div>}>
      <Form />
    </Suspense>
  );
}
