          ...
          {steps.map((step, i) => {
            ...
            return (
              <motion.div
                ...
                className="relative group bg-[#0c1d32] border border-white/[0.08] rounded-2xl p-5 hover:border-dlx-gold/25 transition-all duration-300 hover:shadow-[0_4px_30px_rgba(240,165,0,0.08)]"
              >
                {/* Connector */}
                {i < steps.length - 1 && (
                  <div className="hidden lg:block absolute top-9 left-[calc(50%+32px)] right-[-50%] h-px bg-gradient-to-r from-dlx-gold/20 to-transparent z-0 pointer-events-none" />
                )}

                <div className="flex items-start justify-between mb-4">
                  <div className="w-11 h-11 rounded-xl bg-dlx-gold/[0.10] border border-dlx-gold/[0.18] flex items-center justify-center text-dlx-gold group-hover:bg-dlx-gold/[0.16] transition-colors">
                    <Icon size={20} />
                  </div>
                  <span className="text-[22px] font-bold text-white/[0.06] tabular-nums leading-none mt-0.5">
                    {step.num}
                  </span>
                </div>
                ...
                <p className="text-[#4a6e8f] text-xs leading-relaxed">{step.desc}</p>
              </motion.div>
            );
          })}
        </div>
